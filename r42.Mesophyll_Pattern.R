...
cl$size
## Export gene cluster
write.table(cl$cluster, "Mesophyll/Mesophyll_clusters.txt", 
            quote = F, row.names = T, col.names = F, sep = "\t")

## Draw line chart
pdf("Mesophyll/Mesophyll_patterns.pdf", width = 6, height = 7)
#p <- 
...
for (i in 1:length(file)) {
  dir.create(paste("Mesophyll/Enrichment/",i,sep = ""))
  ...
  write.table(ego.all,paste("Mesophyll/Enrichment/",i,"/Cluster",i,"_GO.results.txt",sep = ""),row.names = FALSE,sep = "\t")
  ...
  write.table(ekk.all, paste("Mesophyll/Enrichment/",i,"/Cluster",i,"_KEGG.results.txt",sep = ""),row.names = FALSE,sep="\t")
  #   
}
